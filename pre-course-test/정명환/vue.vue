<template>
  <div>
    ...
    <p v-bind:style="[login ? {display : 'block'} : {display : 'none'}]">user has been logged in</p>
    ...
    <HelloVue v-bind:val="num"/>

    <!-- 문제 5 -->
    <!-- 문제 3에서 등록한 컴포넌트에서 이벤트를 발생시켜 num 데이터의 값을 변경해보세요 -->
    <HelloVue v-bind:val="num" v-on:change="changeNum"/>

  </div>
</template>

<script>
import HelloVue from '' HelloVue from './components/HelloVue';
